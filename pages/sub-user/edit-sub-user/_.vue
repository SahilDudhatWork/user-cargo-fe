<template>
  <div>
    <div class="lg:mx-40 mx-6 mt-10">
      ...
      <form class="space-y-4 md:space-y-6 mt-6" @submit.prevent="editSubUser">
        <div>
          <div
...
export default {
  ...
  computed: {
    ...mapGetters({
      getSingleSubUserData: "sub-user/getSingleSubUserData",
    }),
  },
  methods: {
    ...mapActions({
      updateSubUser: "sub-user/updateSubUser",
      fetchSingleSubUser: "sub-user/fetchSingleSubUser",
    }),
    togglePassword() {
    ...
    async editSubUser() {
      try {
        ...
        const response = await this.updateSubUser(this.formData);
        this.$toast.open({
  ...
  async asyncData({ params, store, redirect }) {
    ...
    try {
      await store.dispatch("sub-user/fetchSingleSubUser", { accountId: id });
    } catch (error) {
      return redirect("/sub-user");
    }
  },
  beforeMount() {
    this.formData = this.$lodash.cloneDeep(this.getSingleSubUserData);
  },
};
</script>
